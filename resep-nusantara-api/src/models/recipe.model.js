...
async function list({ category, region, difficulty, page, limit }) {
  ...
  const dataResult = await pool.query(
    `SELECT id, name, category, region, difficulty, cook_time_minutes, servings, description, created_at
     FROM recipes
     ${whereClause}
     ORDER BY id ASC
...
async function findIngredientsByRecipeId(recipeId) {
  const result = await pool.query(
    `SELECT ri.id, ri.quantity, ri.unit, ri.notes,
            i.id AS ingredient_id, i.name AS ingredient_name, i.default_unit
